  ...
  useEffect(() => {
    const fetchMentors = async () => {
      try {
        ...
        setExploreMentors(extractCollection(res.data));
      } catch (err) {
        console.error(err);
    ...
    <div className="min-h-screen transition-smooth duration-300">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 px-6">
        {exploreMentors.map((item) => (
          <div
            key={item._id}
            className="bg-white rounded-xl shadow-sm hover:shadow-lg hover:-translate-y-2 transition-all duration-300 overflow-hidden group p-4"
          >
            <img className="w-full h-60 object-cover rounded-lg mb-4" src={item.image} alt={item.name} />
            <h2 className="text-2xl font-bold">{item.name}</h2>
            <h4 className="text-lg text-gray-700">
              {item.company}
              <span className="text-sm text-gray-500 ml-2">{item.experience}</span>
            </h4>
            ...
            <div className="text-sm text-gray-700 mt-2">
              ...
              <div className="flex flex-wrap gap-2">
                {(item.technologies || []).map((skill, index) => (
                  <span
                    key={index}
                    className="px-2.5 py-1 bg-blue-50 text-gray-700 text-xs rounded-md font-medium"
                  >
                    {skill}
                  </span>
                ))}
