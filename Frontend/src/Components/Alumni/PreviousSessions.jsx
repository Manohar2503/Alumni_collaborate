...
const PreviousSessions = () => {
    ...
    const fetchPreviousSessions = async () => {
      try {
        const res = await axios.get(
          `${import.meta.env.VITE_REACT_APP_API_URL}/mentors/sessions/previous`
        );
  ...
  const allTopics = [
    "All Topics",
    ...new Set(safeData.flatMap((session) => (Array.isArray(session.topics) ? session.topics : []))),
  ];
  const allCompanies = [
    "All Companies",
    ...new Set(
      safeData.flatMap((session) => (session.mentor || []).map((mentor) => mentor.company).filter(Boolean))
    ),
  ];
  ...
  const filteredData = safeData.filter((session) => {
    ...
    const matchesCompany =
      ...
      (session.mentor || []).some((mentor) => mentor.company === selectedCompany);
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
          ...
          <div className="mt-4 text-sm text-gray-600">
            Showing <span className="font-semibold text-gray-800">{filteredData.length}</span> of {safeData.length} sessions
          </div>
        </div>
                ...
                <div className="flex-grow">
                  ...
                  <div className="bg-gray-50 p-3 rounded-lg mb-4 space-y-2 text-sm">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-600">{item.mentor?.[0]?.name || "Mentor"}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-gray-700">Role:</span>
                      <span className="text-gray-600">{item.mentor?.[0]?.role || "Mentor"}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <span className="font-semibold text-gray-700">Company:</span>
                      <span className="text-gray-600">{item.mentor?.[0]?.company || "N/A"}</span>
                    </div>
                  </div>

                  <div className="flex flex-wrap gap-2">
                    {(item.topics || []).map((topic, index) => (
                      <span key={index} className="px-2.5 py-1 bg-blue-50 text-blue-700 text-xs rounded-md font-medium">
                        {topic}
                      </span>
                    ))}
                  </div>
