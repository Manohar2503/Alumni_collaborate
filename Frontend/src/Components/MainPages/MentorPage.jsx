  ...
  return (
    <div>
      <div className="bg-gray-100 rounded-lg p-4 text-center shadow-lg">
            ...
            <div>
            <button className="bg-blue-800 text-white m-3 px-4 py-2 rounded hover:bg-blue-600 rounded-lg"><a href={data.video} target="_blank">watch</a></button>
          </div>
          </div>
    </div>
  );
...
const [eventData, setEventData] = useState([]);
  ...
  useEffect(() => {
    ...
    const getEventsData = async () => {
      try {
        const response = await fetch('http://localhost:5000/api/newMentor', {
          method:'GET',
          headers :{
            'Content-Type': 'application/json',
          }
