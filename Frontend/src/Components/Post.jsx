...
export default function Post({ data }) {
  ...
  return (
    <>
      {/* ✅ POST CARD */}
      <div
        className="bg-white rounded-lg border border-gray-200 mb-4 cursor-pointer hover:shadow-sm transition"
    ...
    {data.media.length > 1 && (
      <>
        ...
        {showComments && (
          <div
            className="px-4 pb-4"
            onClick={(e) => e.stopPropagation()} // ✅ clicking inside comments should not open modal
          >
            {comments.map((c, i) => (
      ...
      {openModal && (
        <PostModal post={data} onClose={() => setOpenModal(false)} />
      )}
    </>
  );
}
